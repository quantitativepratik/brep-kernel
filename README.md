...
- Boolean intersection graph:
  - enumerates every left/right face pair across two solids
  - records disjoint, touching, intersecting, coincident, and unsupported pair status
  - stores non-empty adjacency plus trim-ready intersection curve records with model-space curves and per-face p-curves where available
- Boolean topology merge analysis:
  - detects cross-operand coincident and near-coincident vertices, edges, and faces
  - records edge overlaps, point tangencies, coplanar face overlaps, and nearly coincident face pairs
...
- Healed Boolean output:
  - assembles generalized kept-region output via `build_closed_boolean_output`
  - emits validated closed `Solid`s when classified kept regions form a watertight shell
  - promotes supported classified split sides into new trim loops
  - triangulates healed regions, runs tolerance-aware sewing, removes tiny-edge/sliver/duplicate triangles, and validates a new half-edge solid when the generated shell closes
  - returns repaired meshes plus topology diagnostics when arbitrary inputs cannot be made manifold
- Boolean operation:
...
The boolean module deliberately supports one hard representative closed-output case instead of pretending to solve all solid modeling. The result is a real closed half-edge solid for cube-minus-cylinder with `V - E + F = 0`, which is the expected genus-1 topology. For the general pipeline, the module can now build a total face-pair intersection graph across two solids, including disjoint pairs, adjacency for non-empty pairs, analytic NURBS curve records where supported, and finite triangle-face fallback segments for current half-edge solids. A tolerance-aware topology merge pass records coincident and near-coincident vertices, edges, and faces, including tangent point contacts, collinear edge overlaps, coplanar overlaps, and nearly coincident face pairs. Trim-ready SSI curves can be gap-closed and promoted into staged split records or closed inner trim loops. The region classifier partitions affected faces by multiple active split curves, classifies all emitted regions, and includes unsplit faces when the opposite labelled operand is closed enough for point-in-solid testing. `build_closed_boolean_output` triangulates all kept regions, heals the mesh, and returns a validated closed `Solid` whenever the kept region set forms a watertight shell. Supported boundary-to-boundary splits can also be promoted into healed trim regions, triangulated, sewn, filtered for slivers/tiny edges/duplicates, and submitted to the half-edge validator. If arbitrary input cannot become a closed manifold, the API returns the repaired mesh plus the topology error instead of panicking. Persistent IDs, revision history, transaction rollback, and per-entity tolerances are maintained across local metadata, p-curve, trim, and staged split edits. Full feature-history propagation through arbitrary boolean shell rewrites, global trim-loop shell rewriting, and general curved arrangement construction are still outside the supported scope.
