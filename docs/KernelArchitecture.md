...
The compact topology handles (`VertexId`, `EdgeId`, `FaceId`, and related ids) are snapshot indices for fast array access. Each `Solid` also carries a `TopologyIdentity` table with stable `PersistentId`s for vertices, half-edges, edges, staged split edges, faces, and shells. The identity table is validated with the topology arrays, rejects duplicate or mismatched id kinds, and records revisioned `TopologyHistoryEvent`s for construction, sewing, support-surface edits, trim edits, p-curve generation, tolerance edits, and staged split creation. This gives local topology mutations persistent identity and ancestry without forcing every algorithm to abandon compact index-based storage.

The tolerance model is also aligned with the topology arrays. `TopologyToleranceModel` carries separate records for vertices, directed coedges, undirected edges, and faces. Vertex tolerances represent model-space positional uncertainty; coedge tolerances represent both model-space and parameter-space uncertainty for one face-side use of an edge; edge tolerances distinguish endpoint matching from curve fitting/evaluation; face tolerances distinguish surface, trim, and normal/angular classification tolerances. The validator checks table alignment and finite nonnegative values, and edge/trim validation consults these records in addition to legacy local tolerances.

Topology edits can be grouped through `TopologyTransaction`. A transaction snapshots the original solid, routes edits through the checked mutation APIs, collects rollback entries from the generated history events, and either commits the validated result or restores the original snapshot. Dropping an uncommitted transaction rolls back automatically. This is still a snapshot-backed transaction layer rather than a minimal journal replay engine, but it gives callers an explicit atomic boundary and an auditable rollback log for multi-step topology edits.
...
The topology layer also has a staged face-splitting representation: a `SplitEdge` stores the shared 3D split curve outside the closed shell graph, and each participating face records a `FaceSplit` p-curve in its parameter domain. This lets SSI output be installed for later Boolean classification without corrupting the shell's Euler characteristic. Creating a staged split allocates a new persistent split-edge id and records the two source face ids as parents; when done inside a transaction, rollback removes the created split and restores the original identity/history state. A tolerance-aware sewing constructor clusters near-coincident mesh vertices, drops triangles collapsed by sewing, reports the input-to-output vertex map, and then validates the sewn shell with the same half-edge checks as exact construction. The next layers are propagating feature-level naming through arbitrary boolean output generation, promoting more staged split graphs into rewritten trim loops, and full shell topology mutation.
